...
namespace Mkey
{
    ...
    public class SoftEffects : MonoBehaviour
    {
        ...
        internal void RenderNewTextures(GPUWorker gpuWorker, bool rebuild)
        {
            ...
            {
                // shadowOptions.RenderTexture_AR8(gpuWorker, faceOptions.mainTexture); // 10.06.2026
            }
            else
            {
        ...
        public void CreateWorkMaterial()
        {
            ...
            {
                // material = (!shadowOptions.use) ? new Material(Shader.Find("SoftEffects/SoftShaderFont")) : new Material(Shader.Find("SoftEffects/SoftShaderFontWithShadows")); // 10.06.2026
                material = new Material(Shader.Find("SoftEffects/SoftShaderFont")) ;
                material.SetTexture("_MainTex", face);  // 11.06.2026
                AssetDatabase.CreateAsset(material, folderPath + "/" + SoftMaterial.name + "_work" + ".mat");
            }
            else
            {
                // material = (!shadowOptions.use) ? new Material(Shader.Find("Sprites/Default")) : new Material(Shader.Find("SoftEffects/SoftShaderSpriteWithShadows"));       // 10.06.2026
                material = new Material(Shader.Find("Sprites/Default"));
                AssetDatabase.CreateAsset(material, folderPath + "/" + SoftMaterial.name + "_work" + ".mat");
            }

            /* 
            if (shadowOptions.use) // 10.06.2026
            {
